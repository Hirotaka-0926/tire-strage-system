...
const PitCustomerInfoCard = ({ selectedTask }: PitCustomerInfoCardProps) => {
  return (
    <Card>
      ...
      <CardContent>
        <div className="font-medium">{selectedTask?.client?.client_name || "未設定"}</div>
        <div className="text-sm text-gray-600">{selectedTask?.car?.car_model || "未設定"}</div>
        <div className="text-sm text-gray-600">{selectedTask?.car?.car_number || "未設定"}</div>
      </CardContent>
    </Card>
  );
};
