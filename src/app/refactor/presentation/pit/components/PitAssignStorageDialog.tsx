...
import { AlertTriangle, History, Loader2, MapPin, Save, User } from "lucide-react";
import {
  PitStorage,
  PitStorageAssignmentOptions,
  PitTaskInput,
} from "@/app/refactor/domain/type/pit";
import { createPitStorageAssignmentApplication } from "@/app/refactor/application/pit/pitStorageAssignmentApplication";
...
const PitAssignStorageDialog = ({
  ...
  const [selectedStorage, setSelectedStorage] = useState<PitStorage | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [isAssigning, setIsAssigning] = useState(false);

  const isOccupied = Boolean(
    selectedStorage &&
      options.occupiedOptions.find((storage) => storage.id === selectedStorage.id),
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setIsLoading(true);
      try {
        const nextOptions = await app.getAssignmentOptions(selectedTask.client!.id!);
  ...
  const handleSelectStorage = (storageId: string) => {
    const found = allOptions.find((storage) => storage.id === storageId) ?? null;
        ...
        <div className="space-y-4 py-2">
            ...
            <div className="rounded-lg border border-blue-200 bg-blue-50 p-3">
              ...
              <div className="space-y-1">
                {options.customerHistory.slice(0, 3).map((log, index) => (
                  <div key={`${log.storage.id}-${index}`} className="text-sm text-blue-700">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-gray-500">
              空き: {options.emptyOptions.length} / 使用中: {options.occupiedOptions.length}
            </p>
          </div>
