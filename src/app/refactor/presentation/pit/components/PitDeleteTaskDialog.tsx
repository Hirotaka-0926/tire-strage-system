...
import { createPitTaskDeleteApplication } from "@/app/refactor/application/pit/pitTaskDeleteApplication";
      ...
      <DialogContent className="sm:max-w-[480px]">
        ...
        <div className="space-y-2 py-2">
          ...
          <div className="text-lg font-semibold">
            {selectedTask?.id ? `#${selectedTask.id.toString().padStart(3, "0")}` : "#未割当"}
          </div>
          <div className="mt-2 grid grid-cols-1 gap-2 text-sm">
            ...
            <div>
              ...
              <span className="font-medium">{selectedTask?.car?.car_model || "-"}</span>
            </div>
            <div>
              <span className="text-gray-600">ナンバー: </span>
              <span className="font-medium">{selectedTask?.car?.car_number || "-"}</span>
            </div>
          </div>
        </div>
