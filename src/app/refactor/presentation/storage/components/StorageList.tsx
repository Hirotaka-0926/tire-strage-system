...
export const StorageList = ({ slots, selectedSlot, onSlotSelect }: StorageListProps) => {
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className={`h-3 w-3 rounded ${isOccupied(slot) ? "bg-red-500" : "bg-green-500"}`} />
                <span className="font-medium">{slot.id}</span>
                <Badge variant="outline">{isOccupied(slot) ? "使用中" : "空き"}</Badge>
              </div>
              {isOccupied(slot) && <div className="text-sm text-gray-600">使用中</div>}
            </div>
          </div>
        ))}
      </div>
