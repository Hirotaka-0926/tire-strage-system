"use client";

import { BarChart3 } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import type { StorageAreaConfig, StorageSlot } from "@/app/refactor/domain/type/storage";

interface StorageStats {
  total: number;
  available: number;
  occupied: number;
}

interface StatsCardsProps {
  areas: StorageAreaConfig[];
  slots: StorageSlot[];
}

export const StatsCards = ({ areas, slots }: StatsCardsProps) => {
  const getAreaStats = (areaName: string): StorageStats => {
    const areaSlots = slots.filter((slot) => slot.id.startsWith(`${areaName}_`));
    return {
      total: areaSlots.length,
      available: areaSlots.filter(
        (s) => s.car_id === null && s.client_id === null && s.tire_state_id === null
      ).length,
      occupied: areaSlots.filter(
        (s) => s.car_id !== null || s.client_id !== null || s.tire_state_id !== null
      ).length,
    };
  };

  const totalStats: StorageStats = {
    total: slots.length,
    available: slots.filter(
      (s) => s.car_id === null && s.client_id === null && s.tire_state_id === null
    ).length,
    occupied: slots.filter(
      (s) => s.car_id !== null || s.client_id !== null || s.tire_state_id !== null
    ).length,
  };

  const totalRate = totalStats.total > 0 ? Math.round((totalStats.occupied / totalStats.total) * 100) : 0;
  const availableRate = totalStats.total > 0 ? Math.round((totalStats.available / totalStats.total) * 100) : 0;

  return (
    <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
      {areas.map((area) => {
        const stats = getAreaStats(area.name);
        return (
          <Card key={area.name}>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-gray-600">
                エリア{area.name} ({area.totalSlots}枠)
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="flex items-center">
                    <div className="mr-2 h-3 w-3 rounded bg-green-500" />
                    空き
                  </span>
                  <span className="font-semibold">{stats.available}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="flex items-center">
                    <div className="mr-2 h-3 w-3 rounded bg-red-500" />
                    使用中
                  </span>
                  <span className="font-semibold">{stats.occupied}</span>
                </div>
              </div>
            </CardContent>
          </Card>
        );
      })}

      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="flex items-center text-sm font-medium text-gray-600">
            <BarChart3 className="mr-2 h-4 w-4" />
            全体統計
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span>使用率</span>
              <span className="font-semibold">{totalRate}%</span>
            </div>
            <div className="flex justify-between text-sm">
              <span>空き率</span>
              <span className="font-semibold">{availableRate}%</span>
            </div>
            <div className="flex justify-between text-sm">
              <span>総枠数</span>
              <span className="font-semibold">{totalStats.total}</span>
            </div>
          </div>
        </CardContent>
      </Card>
    </div>
  );
};
