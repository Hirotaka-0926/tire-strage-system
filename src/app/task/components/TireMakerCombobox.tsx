...
import { getTireMakersFromPrice } from "@/utils/supabaseFunction";
...
const TireMakerCombobox = ({ value, onChange, disabled, label = "タイヤメーカー" }: Props) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const list = await getTireMakersFromPrice();
...
export default TireMakerCombobox;
